                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <el-button
                            size="mini"
                            type="danger"
                            @click="deleteOnlineJudgeSystem(scope.row)"
                            round>删除
                        </el-button>
                    </template>
                </el-table-column>
...
<script>
import {listOnlineJudgeSystemPage, updateOnlineJudgeSystem, deleteOnlineJudgeSystem} from '@/network/api/report';
...
export default {
    ...
    methods: {
        ...
        deleteOnlineJudgeSystem(row){
            const onlineJudgeSystem = {
                id: row.id,
            }
            deleteOnlineJudgeSystem(onlineJudgeSystem).then(res => {
                if (res.code !== 200) {
                    return this.$message.error(res.message);
                }
                this.getList();
